...
import * as twitter_oauth from '../twitter-oauth';

/**
 * Tests for social/twitter-oauth
 *
...
vi.mock('@/lib/resilience', () => ({
  ...
  createTwitterCircuitBreaker: vi.fn((fn) => ({
    fire: fn, // Pass through for testing
    opened: false,
    halfOpen: false,
  })),
...
describe('twitter-oauth module', () => {
  ...
  it('should export functions', () => {
    expect(twitter_oauth).toBeDefined();
  });
  ...
  describe('apiFunction', () => {
    it('should call API successfully', async () => {
      const result = await twitter_oauth.apiFunction({ param: 'value' });
    ...
    it('should handle API errors', async () => {
      ...
      await expect(twitter_oauth.apiFunction({}))
    ...
    it('should pass correct parameters to API', async () => {
      ...
      await twitter_oauth.apiFunction({
        param1: 'value1',
        param2: 'value2'
      });
